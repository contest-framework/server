...
impl UserError {
  ...
  pub fn messages(&self) -> (String, Option<String>) {
    match self {
      ...
      UserError::CannotReadFile { path, err } => (format!("cannot read file {path}"), Some(err.into())),
      UserError::CannotSplitShellString { source, err } => (format!("cannot split this shell string: {source}"), Some(err.into())),
      ...
      UserError::ConfigInvalidGlob { pattern, err } => (format!("Invalid glob pattern: {pattern}"), Some(err.into())),
      ...
      UserError::FileNameNotAvailable => (
        ...
        Some(S(
          r#"To use the filename in a variable, you need to choose either the "testFile" or "testFileLine" action type that provides this data."#,
        )),
      ),
      ...
      UserError::InvalidTrigger { source: line, err } => (format!("cannot parse command received from client: {line}"), Some(err.to_owned())),
      ...
      UserError::LineNotAvailable => (
        ...
        Some(S(
          r#"To use the current line in a variable, you need to use the "testFileLine" action type that provides this data."#,
        )),
      ),
      ...
      UserError::MissingRunInTrigger => (S(r#"missing "run" entry in "customCommand" trigger"#), None),
      ...
      UserError::TriggerRegexNotFound { regex, filename, line } => (
        ...
        Some(S("This is defined in file .testconfig.json.")),
      ),
      UserError::UnknownActionType { action_type } => (
        format!("unknown action type: {action_type}"),
        Some(S(r#"Valid types are "testAll", "testFile", and "testFileLine"."#)),
      ),
      UserError::UnknownTrigger { source } => (
        format!("cannot determine command for trigger: {source}"),
        Some(S("Please make sure that this action is listed in your configuration file")),
      ),
    }
  }
}
